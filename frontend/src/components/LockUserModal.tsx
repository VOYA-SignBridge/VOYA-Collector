    ...
    <div className="fixed inset-0 z-[9998] flex items-center justify-center bg-slate-900/60 backdrop-blur-sm p-4" onClick={onClose}>
      <div className="max-w-lg w-full bg-white rounded-2xl shadow-2xl border border-slate-200 p-6" onClick={(e) => e.stopPropagation()}>
        <div className="flex items-center gap-2 mb-1">
          ...
          <h3 className="text-lg font-bold text-slate-900">{t("Khóa tài khoản")}</h3>
        </div>
        <p className="text-sm text-slate-500 mb-4">
          <Trans
            k="Khóa tài khoản {ten}. Người dùng sẽ bị đăng xuất và thấy thông báo kèm lý do bên dưới."
            vars={{
              ten: <span className="font-semibold text-slate-700">{username}</span>,
            }}
          />
        </p>
        ...
        <label className="block text-xs font-semibold text-slate-500 uppercase tracking-wide mb-2">{t("Thời hạn khóa")}</label>
