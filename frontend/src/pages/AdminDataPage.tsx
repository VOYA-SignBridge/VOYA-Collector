...
function BarList({ title, icon, rows }: { title: string; icon: ReactNode; rows: { name: string; count: number }[] }) {
  ...
  return (
    <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-5">
      ...
      {rows.length === 0 ? (
        <p className="text-sm text-slate-400">{t("Chưa có dữ liệu.")}</p>
      ) : (
    ...
    <div className="space-y-6 animate-fade-in">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-slate-600">{t("Báo cáo & thống kê dữ liệu đóng góp, quản lý đồng bộ lưu trữ")}</p>
        </div>
        <button onClick={() => setSyncOpen(true)}
          title={t("Kiểm tra & tải các file còn thiếu từ Google Drive về server (không thay đổi cơ sở dữ liệu)")}
          ...
          <CloudDownloadIcon className="w-5 h-5" /> {t("Đồng bộ dữ liệu (Drive → server)")}
        </button>
      </div>
      ...
      {loading ? (
        <div className="py-20">
          <LoadingSpinner size="lg" label={t("Đang tải báo cáo dữ liệu...")} />
        </div>
      ) : (
        <>
          {/* Totals */}
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
            <StatCard label={t("Nhãn")} value={data?.labels_count ?? 0} icon={<TagIcon className="w-5 h-5" />} />
            <StatCard label={t("Mẫu dữ liệu")} value={data?.total_samples ?? 0} icon={<CameraIcon className="w-5 h-5" />} />
            <StatCard label={t("Người đóng góp")} value={data?.contributors_count ?? 0} icon={<UsersIcon className="w-5 h-5" />} />
            <StatCard label={t("Vùng miền")} value={data?.regions_count ?? 0} icon={<GlobeIcon className="w-5 h-5" />} />
          </div>

          {/* Breakdowns */}
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
            <BarList title={t("Top nhãn theo số mẫu")} icon={<TagIcon className="w-5 h-5" />} rows={(data?.top_labels ?? []).map((r) => ({ name: r.label, count: r.count }))} />
            <BarList title={t("Phân bố theo vùng")} icon={<GlobeIcon className="w-5 h-5" />} rows={(data?.by_region ?? []).map((r) => ({ name: r.region, count: r.count }))} />
            <BarList title={t("Người đóng góp nhiều nhất")} icon={<UsersIcon className="w-5 h-5" />} rows={(data?.top_contributors ?? []).map((r) => ({ name: r.username, count: r.count }))} />
            <BarList title={t("Theo nguồn")} icon={<DatabaseIcon className="w-5 h-5" />} rows={(data?.by_source ?? []).map((r) => ({ name: r.source, count: r.count }))} />
          </div>

          {/* Recent */}
          <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-5">
            <h3 className="font-semibold text-slate-700 mb-4 flex items-center gap-2"><span className="text-ctu-blue"><ClockIcon className="w-5 h-5" /></span>{t("Mẫu mới nhất")}</h3>
            <div className="overflow-x-auto">
              <table className="w-full text-left text-sm">
                <thead>
                  <tr className="text-slate-500 border-b border-slate-200">
                    <th className="py-2 px-3 font-semibold">{t("Nhãn")}</th>
                    <th className="py-2 px-3 font-semibold">{t("Vùng")}</th>
                    <th className="py-2 px-3 font-semibold">{t("Nguồn")}</th>
                    <th className="py-2 px-3 font-semibold">{t("Thời gian")}</th>
                  </tr>
                </thead>
                <tbody>
                  {(data?.recent ?? []).length === 0 ? (
                    <tr><td colSpan={4} className="py-6 text-center text-slate-400">{t("Chưa có mẫu nào.")}</td></tr>
                  ) : (
                    data!.recent.map((r, i) => (
