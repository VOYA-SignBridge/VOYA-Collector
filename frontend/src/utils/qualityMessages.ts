...
export function qcMessage(code: string, detail?: Record<string, unknown>): string {
  ...
  const handsText = hands !== undefined ? HAND_WORD[hands] ?? `${hands} tay` : "đủ tay";
  ...
  switch (code) {
    case "MISSING_REQUIRED_HAND":
      return pct !== undefined
        ? `Chỉ phát hiện ${handsText} ở ${pct}% khung hình — ký hiệu này cần ${handsText}. Mẫu vẫn được lưu nhưng sẽ bị đánh dấu để kiểm tra.`
        : `Không phát hiện đủ ${handsText} trong phần lớn khung hình. Mẫu vẫn được lưu nhưng sẽ bị đánh dấu để kiểm tra.`;
    case "HIGH_JITTER":
    ...
    case "QC_HANDS_MISSING":
      return `Mẫu bị từ chối: ký hiệu này cần ${handsText} nhưng hầu như không phát hiện đủ. Vui lòng thu lại.`;
