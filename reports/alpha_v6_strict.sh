...
OUT=/workspace/processed/train_utils/outputs/alpha_v6_strict
LOG=/workspace/reports/alpha_v6_strict_raw.txt
  ...
  for m in tcn lstm hdgcn bigru_attention cnn; do
    for s in 42 43 44; do
      line=$(python -m processed.train_utils.train_tcn --model_type=$m --seed=$s \
        --run-purpose research --recognition_profile alphabet --features_root /dataset/features \
