...
đề cương `docs/MNM_Thesis proposal_CT553H.docx`.*
...
|---|---|---|
| Bảng CSDL | **44** | `docs/db/schema_erd.sql` |
...
| Test backend | **1.696 xanh / 0 đỏ** (bản sao sản xuất) | `docs/TESTING.md` |
| Test backend trên CSDL dựng-từ-số-không | **1.681 xanh / 0 đỏ / 15 skip** | CI `signdb_ci` |
| Test frontend | **363** (45 tệp) | `docs/TESTING.md` |
...
Khẳng định trung tâm, chép nguyên từ `docs/needFix/TENANT_ISOLATION.md`:
...
| 3.3 Thiết kế CSDL | ERD 44 bảng; **giải thích các cặp khái niệm không được gộp** | `docs/db/schema_erd.sql`, `docs/CONCEPTS.md` |
| 3.4 Thiết kế cô lập tenant | bốn tầng ở Trụ 1; bảng so sánh 3 mô hình cô lập và lý do chọn shared-schema+RLS | `docs/needFix/TENANT_ISOLATION.md` |
| 3.5 Thiết kế IAM | token, phiên, 2FA, cổng mặc-định-từ-chối, hai vòng quyền | `docs/SESSION_LIFECYCLE.md`, `docs/TWO_FACTOR.md` |
| 3.6 Danh mục ba mặt phẳng | System Catalog / Commons / Tenant | `docs/needFix/REGISTRY_ARCHITECTURE.md` |
...
| 3.9 Đồng thuận & quy kết | ba mức, bốn nghĩa thu hồi, văn bản bất biến | `docs/CONSENT_ENFORCEMENT.md`, `docs/LEGAL_DOCUMENTS.md` |
| 3.10 Thiết kế tham chiếu (chưa hiện thực) | Workspace/Project, quota tự động, Commons | `docs/needFix/MULTITENANT_ARCHITECTURE.md` + 8 ADR |
...
5. **Vẽ lại 4 sơ đồ**: triển khai 13 dịch vụ · luồng dữ liệu · ERD rút gọn theo cụm ·
   bốn tầng cô lập. `docs/db/voya_erd.drawio` đã có sẵn nền cho cái thứ ba.
...
| Cần viết về | Đọc |
|---|---|
| Ranh giới khái niệm dễ nhầm | `docs/CONCEPTS.md` ← **đọc trước tiên, chất lượng luận văn** |
| Cô lập tenant | `docs/needFix/TENANT_ISOLATION.md`, `backend/app/storage/rls.py` |
| Kiến trúc đa tenant đầy đủ | `docs/needFix/MULTITENANT_ARCHITECTURE.md` (1.412 dòng) |
| Danh mục & registry | `docs/needFix/REGISTRY_ARCHITECTURE.md` |
| Commons (thiết kế, chưa dựng) | `docs/needFix/COMMUNITY_DATA_COMMONS.md` |
| SOT | `backend/app/sot/__init__.py` + `catalog_schema.py` |
| Kiểm thử | `docs/TESTING.md` |
| Đồng thuận / pháp lý | `docs/CONSENT_ENFORCEMENT.md`, `docs/LEGAL_DOCUMENTS.md` |
| Quan trắc | `docs/OBSERVABILITY_PLAN.md` |
| Sao lưu | `docs/BACKUP_RESTORE.md` |
| Quyết định kiến trúc | `Extra_docs/01-architecture/adr/ADR-001..008` |
| Hạn chế đã biết | `docs/KNOWN_ISSUES.md` (49 KB — mỏ vàng cho phần Hạn chế) |
