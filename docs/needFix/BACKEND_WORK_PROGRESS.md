...
**Tham chiếu kỹ thuật A2+A3:** [TENANT_ISOLATION.md](TENANT_ISOLATION.md) — kiến trúc, luồng nghiệp vụ, vận hành
...
[TENANT_ISOLATION.md](TENANT_ISOLATION.md). Mục này ghi *đã làm gì, tìm thấy gì, đo được gì*.
...
3. **`system_scope` bắt buộc có `reason`.** Biến `grep -rn "system_scope("` thành một bản
   ...
   [TENANT_ISOLATION.md](TENANT_ISOLATION.md) §4.5.
...
Ba mục ở §4 của [TENANT_LIFECYCLE_AND_OTP.md](TENANT_LIFECYCLE_AND_OTP.md) từng được ghi là
