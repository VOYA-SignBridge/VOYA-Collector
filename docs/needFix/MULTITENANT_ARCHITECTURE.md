...
| tài liệu | vai trò |
|---|---|
| [`MULTITENANT_PREP.md`](MULTITENANT_PREP.md) | ghi 2026-07-31 — nền `tenant_id` **đã đặt** trong `metadata_db.py`, và danh sách chỗ còn giả định một tenant. Vẫn đúng, tài liệu này **mở rộng** chứ không thay thế. |
...
| [`AUTH_TOKEN_LIFECYCLE.md`](AUTH_TOKEN_LIFECYCLE.md) | các lỗ hổng vòng đời token; §19 phụ thuộc vào việc vá xong. |
| `docs/OBSERVABILITY_PLAN.md` | Loki label bounded + audit ở Postgres. §18 áp dụng nguyên tắc đó. |
...
phải Loki — đúng theo `docs/OBSERVABILITY_PLAN.md`: Loki giữ chi tiết kỹ thuật,
